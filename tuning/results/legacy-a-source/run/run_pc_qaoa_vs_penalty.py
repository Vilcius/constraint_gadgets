"""
run_pc_qaoa_vs_penalty.py -- Run PC-QAOA vs PenaltyQAOA experiments.
...
from core import pc_qaoa as hq
...
from analyze_results.metrics import p_feasible_pcqaoa
...
_METHOD_ALIASES = {'PC-QAOA': {'PC-QAOA', 'HybridQAOA', 'PCQAOA'}}
...
def _load_resume_state(path: str, method_name: str):
    ...
    try:
        ...
        aliases = _METHOD_ALIASES.get(method_name, {method_name})
        rows = df[m_col.isin(aliases)]
        if rows.empty:
            return None, 0
...
def run_cop(cop: dict, qubos: dict, gadget_db_path: str,
    ...
    if h_start_layer > 0 and verbose:
        print(f'  PC-QAOA resuming from layer {h_start_layer} → {MAX_LAYERS}')
    ...
    for p in range(h_start_layer + 1, MAX_LAYERS + 1):
        ...
        opt_cost, opt_angles = pcqaoa.optimize_angles(
            prev_layer_angles=prev_h_angles,
        )
        counts = pcqaoa.do_counts_circuit(shots=SHOTS)
        _, est_shots, est_error, grp_shots, grp_error = pcqaoa.get_circuit_resources()
        C_min, C_max = ising_hamiltonian_extremes(pcqaoa.problem_ham, pcqaoa.all_wires)
        ...
        row = {
            ...
            'n_structural':           [len(structural_constraints)],
            'n_penalty':              [len(penalty_constraints)],
            'Hamiltonian':            [pcqaoa.problem_ham],
            'angle_strategy':         [pcqaoa.angle_strategy],
            'n_layers':               [pcqaoa.n_layers],
            'num_gamma':              [pcqaoa.num_gamma],
            'num_beta':               [pcqaoa.num_beta],
            ...
            'hamiltonian_time':       [getattr(pcqaoa, 'hamiltonian_time', None)],
            'optimize_time':          [pcqaoa.optimize_time],
            'counts_time':            [pcqaoa.count_time],
            'total_time':             [(getattr(pcqaoa, 'hamiltonian_time', None) or 0.0)
                                       + pcqaoa.optimize_time + pcqaoa.count_time],
            'C_max':                  [C_max],
            'C_min':                  [C_min],
            'min_val':                [min_val],
            ...
            'layer':                  [p],
            'optimal_x':              [optimal_x],
            'cumulative_time':        [h_cumulative_time],
        }
        h_cumulative_time += row['total_time'][0]
        row['cumulative_time'] = [h_cumulative_time]
        pcqaoa_rows.append(row)
        prev_h_angles = jnp.array(opt_angles)

        p_feas = p_feasible_pcqaoa(row)
        if verbose:
            ar = row['AR'][0]
            print(f'  PC-QAOA p={p}: AR={ar:.4f}  P(feas)={p_feas:.4f}')

    if pcqaoa_checkpoint_path is not None and pcqaoa_rows:
        _save_rows(_flatten_rows(pcqaoa_rows, 'PC-QAOA'), pcqaoa_checkpoint_path)
        print(f'  [checkpoint] PC-QAOA rows saved → {pcqaoa_checkpoint_path}',
              flush=True)
    ...
    for p in range(p_start_layer + 1, MAX_LAYERS + 1):
        ...
        row = {
            ...
            'penalty_constraints':    [all_constraints],
            'n_x':                    [pen_solver.n_x],
            'n_c':                    [len(all_constraints)],
            'n_structural':           [0],
            'n_penalty':              [len(all_constraints)],
    ...
    return {'pcqaoa_rows': pcqaoa_rows, 'penalty_rows': penalty_rows}
...
def _merge(pending_dir: str, output: str, data_dir: str = 'data/') -> None:
    ...
    table_prefix = os.path.join(output_dir, 'problem_table')
...
def _parse_args():
    p = argparse.ArgumentParser(
        description='Run PC-QAOA vs PenaltyQAOA layer sweeps.'
    )
    ...
    p.add_argument('--output', default='results/overlapping/pc_qaoa_vs_penalty.pkl')
...
if __name__ == '__main__':
    ...
    if args.cop_id is not None:
        ...
        h_resume_angles, h_start_layer = _load_resume_state(result_path, 'PC-QAOA')
        ...
        print(f'COP {args.cop_id}/{len(cops)-1}  '
              f'(PC-QAOA: layers {h_start_layer+1}-{MAX_LAYERS}, '
        ...
        try:
            result = run_cop(cop, qubos, args.db,
                              pcqaoa_checkpoint_path=result_path,
    ...
    for i, cop in enumerate(cops):
        print(f'\n[{i+1}/{len(cops)}]')
        result = run_cop(cop, qubos, args.db)
        for row in result['pcqaoa_rows']:
            r = dict(row); r['method'] = ['PC-QAOA']; collector.add(r)
