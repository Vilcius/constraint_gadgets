...
from core.penalty_heuristics import practical_penalties, PENALTY_METHODS
...
def settings_b(instance, config):
    cfg = config['study_b']
    if 'multipliers' in cfg or cfg.get('methods') != list(PENALTY_METHODS):
        raise ValueError('Study B requires the five fixed methods, without multipliers')
    anchors = practical_penalties(np.asarray(instance['Q']),
        ch.parse_constraints(instance['constraints']), instance['feasible_point'])
    return [dict(id=name, rule=name, delta=anchors[name], selectable=True)
