...
# ── Hyperparameters (match server runs) ───────────────────────────────────────
MAX_LAYERS        = 5
...
def run_flag_variant(task, Q, parsed, str_idx, pen_idx,
    ...
    for p in range(1, MAX_LAYERS + 1):
        hybrid = hq.HybridQAOA(
            ...
            penalty_str=[penalty_weight], penalty_pen=penalty_weight,
        ...
        print(f'    [flag]   p={p}: P(feas)={m["p_feasible"]:.3f}  P(opt)={m["p_optimal"]:.3f}')
        if m['p_feasible'] >= P_FEAS_THRESHOLD:
            break
    return rows
...
def run_noflag_variant(task, Q, parsed, str_idx, pen_idx,
    ...
    for i in gadget_idxs:
        raw = parsed[i].raw
        print(f'    [noflag] Training VCGNoFlag: {raw}')
        vcg_nf = VCGNoFlag(
            constraints=[raw],
    ...
    if pen_constraints:
        ...
        H_penalty = base.build_penalty_hamiltonian(
            pen_constraints, slack_infos, penalty_weight, x_wires[0])
    else:
        slack_wires = []
        H_penalty = None
    ...
    for p in range(1, MAX_LAYERS + 1):
        ...
        rows.append({'p': p, **m})
        prev_angles = opt_angles
        print(f'    [noflag] p={p}: P(feas)={m["p_feasible"]:.3f}  P(opt)={m["p_optimal"]:.3f}')
        if m['p_feasible'] >= P_FEAS_THRESHOLD:
            break
    return rows
...
for task in FOCUS_CASES:
    ...
    print('=' * 65)
    print(f'Case: {name}  n={n_x}  opt={min_val:.3f}  δ={penalty_weight:.2f}')
    print(f'  Constraints: {all_constraints}')
    print('=' * 65)

    print('  >> VCG (flag):')
    flag_rows = run_flag_variant(
        task, Q, parsed, str_idx, pen_idx, penalty_weight, optimal_x, n_x)
...
for name, res in all_results.items():
    print(f'\n{name}:')
    print(f'  {"Variant":<12} {"p":>3}  {"AR":>7}  {"P(feas)":>9}  {"P(opt)":>8}')
    print('  ' + '-' * 45)
    for row in res['flag']:
        print(f'  {"VCG(flag)":<12} {row["p"]:>3}  {row["AR"]:>7.4f}  {row["p_feasible"]:>9.4f}  {row["p_optimal"]:>8.4f}')
    for row in res['noflag']:
        print(f'  {"VCGNoFlag":<12} {row["p"]:>3}  {row["AR"]:>7.4f}  {row["p_feasible"]:>9.4f}  {row["p_optimal"]:>8.4f}')
