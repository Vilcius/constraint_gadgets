...
MAX_LAYERS=5, P_FEAS_THRESHOLD=0.75, 10k shots).  PenaltyQAOA column shows the
best result from the baseline (any p) for direct comparison.

### Case 1 — `independent_set + knapsack + cardinality`, n=7

Gadgets: `x_0*x_1==0` (VCGNoFlag, AR=1.0, p=1), `2*x_2+1*x_3+4*x_4<=2`
...
| VCGNoFlag            | 1    | 0.3602 | **1.000** | **0.109** |

**VCGNoFlag beats PenaltyQAOA**: P(feas) 1.000 vs 0.495 (+102%), P(opt) 0.109 vs 0.043 (+154%).
...
| VCGNoFlag            | 1    | 0.8106 | **1.000** | **0.665** |

**VCGNoFlag beats PenaltyQAOA**: P(feas) 1.000 vs 0.781 (+28%), P(opt) 0.665 vs 0.009 (+7300%).
The X-gate preparation for the single-feasible-state cardinality constraint
pins the Grover mixer so it strongly concentrates on the unique optimum `x_4=x_5=x_6=1`.
...
| VCGNoFlag            | 1    | 0.5134 | **1.000** | 0.000     |

**VCGNoFlag partially beats PenaltyQAOA**: P(feas) 1.000 vs 0.886 (+13%), but
P(opt) 0.000 vs 0.871 — PenaltyQAOA still wins on optimization quality.
All shots are feasible but evenly spread: with 8/16 feasible assignments the
outer QAOA at p=1 cannot distinguish the optimum.  Needs more layers / budget
(TODO #4).
...
**VCGNoFlag partially beats PenaltyQAOA**: P(feas) 1.000 vs 0.943 (+6%), but
P(opt) 0.000 vs 0.929.  Both Hybrid variants find only feasible states but the
outer QAOA does not concentrate on the optimum at p=1 — same issue as Case 3.

**Overall summary after Change 1**:

| Case | PenaltyQAOA P(feas) | VCGNoFlag P(feas) | PenaltyQAOA P(opt) | VCGNoFlag P(opt) | Verdict |
|------|---------------------|-------------------|--------------------|------------------|---------|
...
VCGNoFlag fixes the feasibility problem entirely (P(feas)=1.0 everywhere at p=1).
In Cases 1–2 this is sufficient to beat PenaltyQAOA on both metrics.
