"""
pc_qaoa.py -- PCQAOA (PC-QAOA) with JAX-jitted optimisation.
...
class PCQAOA:
    """
    PC-QAOA: per-gadget structural state preparation + penalty Hamiltonian.

    Each structural gadget uses its natural mixer:
      - DickeStatePrep  (cardinality equality)  → XY / Ring-XY
      - FlowStatePrep   (flow conservation)     → Ring-XY
      - CardinalityLeqStatePrep (inequality)    → per-gadget Grover
      - VCG             (variational gadget)    → per-gadget Grover
      - CardinalityGeqSingleStatePrep (fixed)   → no mixer
      - remaining free qubits                  → RX
    """
    ...
    def __init__(
        ...
        isp_idxs = [i for i in structural_indices
                    if ch.is_independent_set_pair_compatible(all_constraints[i])]
        ...
        gadget_idxs = [i for i in structural_indices
                       ...
                       and not ch.is_independent_set_pair_compatible(all_constraints[i])
        ...
        # Independent-set pair x_i*x_j==0 → x_i+x_j<=1: use CardinalityLeqStatePrep(k=1)
        for idx in isp_idxs:
            pc = all_constraints[idx]
            var_wires = sorted(pc.variables)
            prep = dsp.CardinalityLeqStatePrep(
                var_wires=var_wires, max_hamming_weight=1,
        ...
        # Preps that have a mixer_circuit (geq is fixed — no mixer)
        self.mixer_preps = (self.dicke_preps + self.flow_preps
                            + self.leq_preps + self.gadget_preps)
        ...
        # Free wires: not covered by any gadget (get RX mixer)
        structured_wire_set = set()
        for prep in self.mixer_preps:
            structured_wire_set.update(prep.var_wires)
        for prep in self.geq_preps:  # fixed — no mixer, but not free
            structured_wire_set.update(prep.var_wires)
        self.free_wires = [w for w in self.all_wires if w not in structured_wire_set]

        # One beta per gadget with a mixer + one per free qubit
        self.num_beta = len(self.mixer_preps) + len(self.free_wires)
    ...
    def circuit(self, angles) -> None:
        """PC-QAOA circuit: state prep, then QAOA layers with per-gadget mixers."""
        gammas, betas = base.split_angles(
            angles, self.num_gamma, self.num_beta, self.angle_strategy
        )

        # --- structural state preparation ---
        ...
        for q in range(self.n_layers):
            ...
            beta_idx = 0
            for prep in self.mixer_preps:
                prep.mixer_circuit(betas[q][beta_idx])
                beta_idx += 1
            for wire in self.free_wires:
                qml.RX(betas[q][beta_idx], wires=wire)
                beta_idx += 1
    ...
    def solve(self) -> Tuple[float, dict, any]:
        """Run the full PC-QAOA: optimise angles, then sample."""
    ...
    def opt_circuit(self) -> None:
        ...
        self.circuit(self.opt_angles)

    # ==================================================================
    # PC-QAOA XY mixer (Dicke groups get XY, others get RX)
    # ==================================================================

    def _apply_pcqaoa_xy_mixer(self, beta_row) -> None:
        """
        Apply a PC-QAOA mixer: XY on structured wires, RX on the rest.
...
# ─────────────────────────────────────────────────────────────────────────────
# Module-level helper (unchanged from pcqaoa_qaoa.py)
# ─────────────────────────────────────────────────────────────────────────────
