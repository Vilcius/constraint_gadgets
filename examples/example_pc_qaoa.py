"""
example_pcqaoa.py -- Three-constraint PC-QAOA vs full PenaltyQAOA.
...
-----------------------------------------------------
  Constraint A  (structural -- Dicke state prep)
      ...
      PC-QAOA prepares this subspace exactly via a log-depth W-state
  ...
  Constraint B  (structural -- VCG gadget)
      ...
      make it NOT Dicke-compatible.  PC-QAOA trains a VCG gadget whose
...
Comparison
----------
  PC-QAOA: A and B structural (exact/gadget), C penalized.
...
Run from the project root:
    python examples/example_pcqaoa.py

Output
------
  Prints per-layer metrics table (AR, P(feasible), P(optimal)) to stdout.
  Saves collected results to examples/results/example_pcqaoa_results.pkl.
  Saves figures to examples/figures/:
    pcqaoa_example_layer_sweep.png  --  AR / P(feas) / P(opt) vs layers
    pcqaoa_example_counts.png       --  measurement distributions at final layer
"""
...
for p in range(1, MAX_LAYERS + 1):
    ...
    opt_cost_pc, opt_angles_pc = pcqaoa.optimize_angles(prev_layer_angles=prev_pc_angles)
    counts_pc = pcqaoa.do_counts_circuit(shots=SHOTS)
    C_min_pc, C_max_pc = ising_hamiltonian_extremes(pcqaoa.problem_ham, pcqaoa.all_wires)
    m_pc = compute_comparison_metrics(counts_pc, float(opt_cost_pc), C_max_pc, C_min_pc,
                                     all_constraints, N_X, optimal_x)
    rows.append({'method': 'PC-QAOA', 'layer': p, **m_pc, 'counts': counts_pc})
    prev_pc_angles = jnp.array(opt_angles_pc)
    print(f"  {p:>2}  {'PC-QAOA':<14} {m_pc['AR']:>8.4f} {m_pc['p_feasible']:>10.4f} {m_pc['p_optimal']:>9.4f}")
...
collector.load('examples/results/example_pcqaoa_results.pkl')
for r in rows:
    collector.add(r)
collector.save('examples/results/example_pcqaoa_results.pkl')
print("Saved results to examples/results/example_pcqaoa_results.pkl\n")
...
plot_layer_sweep(
    df,
    title='PC-QAOA vs PenaltyQAOA — 3-constraint COP, layer sweep',
    save_path='examples/figures/pcqaoa_example_layer_sweep.png',
)
print("Saved: examples/figures/pcqaoa_example_layer_sweep.png")
...
final_pc = next(r for r in reversed(rows) if r['method'] == 'PC-QAOA')
final_p = next(r for r in reversed(rows) if r['method'] == 'PenaltyQAOA')

plot_outcome_distributions(
    counts={'PC-QAOA': final_pc['counts'], 'PenaltyQAOA': final_p['counts']},
    constraints=all_constraints,
    n_x=N_X,
    optimal_x=optimal_x,
    top_n=20,
    ...
    save_path='examples/figures/pcqaoa_example_counts.png',
)
print("Saved: examples/figures/pcqaoa_example_counts.png")
print("\nDone.")
