...
Usage:
    ...
    python run/run_knapsack.py --corp hybrid --max_n 4 --n_layers 1
"""
...
from analyze_results.results_helper import read_typed_csv, collect_vcg_data, collect_hybrid_data, remap_constraint_to_vars, remap_to_zero_indexed


def run_vcg(max_n: int, result_dir: str = './results/',
            data_dir: str = './data/',
            result_file: str = 'knapsack_constraint_results') -> None:
    """Train VCG gadgets on knapsack constraints."""
...
def run_hybrid(max_n: int, n_layers: int = 1,
               ...
               data_dir: str = './data/',
               result_file: str = 'hybrid_knapsack_results',
               constraint_result_file: str = 'knapsack_constraint_results') -> None:
    """Run HybridQAOA on knapsack constraints paired with QUBOs."""
    os.makedirs(result_dir, exist_ok=True)
    df = pd.DataFrame()
    df.to_pickle(f'{result_dir}{result_file}.pkl')
