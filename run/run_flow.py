...
Usage:
    ...
    python run/run_flow.py --corp problem --n_layers 1
"""
...
def run_cqaoa(max_in: int = 3, max_out: int = 3,
              result_dir: str = './results/',
              data_dir: str = './data/',
              result_file: str = 'flow_constraint_results',
              combined: bool = False, single_flag: bool = False,
              decompose: bool = True) -> None:
    """Run VCG on all flow conservation constraints."""
    ...
    # Keep only entries whose total vars fit within max_in + max_out
    all_constraints = [(n, cs) for n, cs in all_constraints
                       if n <= max_in + max_out]
...
def run_pqaoa(max_in: int = 3, max_out: int = 3, n_layers: int = 1,
              result_dir: str = './results/',
              data_dir: str = './data/',
              result_file: str = 'qubo_flow_results',
              constraint_result_file: str = 'flow_constraint_results',
              combined: bool = False, overlap: bool = False,
              single_flag: bool = False, decompose: bool = True) -> None:
    """Run ProblemQAOA on flow constraints paired with QUBOs."""
    os.makedirs(result_dir, exist_ok=True)
    df = pd.DataFrame()
    df.to_pickle(f'{result_dir}{result_file}.pkl')
...
if __name__ == '__main__':
    ...
    if args.corp == 'constraint':
        run_cqaoa(args.max_in, args.max_out,
                  result_dir=args.results_dir, data_dir=args.data_dir)
    elif args.corp == 'problem':
        run_pqaoa(args.max_in, args.max_out, n_layers=args.n_layers,
                  result_dir=args.results_dir, data_dir=args.data_dir)
    else:
