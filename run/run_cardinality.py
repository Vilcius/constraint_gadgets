"""
run_cardinality.py -- Run QAOA experiments for cardinality constraints.

Reads data/cardinality_constraints.csv (format: n; ['x_0 + ... op b']).

Usage:
    python run/run_cardinality.py --corp constraint --max_n 5
    python run/run_cardinality.py --corp problem --max_n 5 --n_layers 1
"""
...
def run_cqaoa(max_n: int, result_dir: str = './results/',
    ...
    """Run VCG on all cardinality constraints up to max_n variables."""
...
def run_pqaoa(max_n: int, n_layers: int = 1,
              ...
              data_dir: str = './data/',
              result_file: str = 'qubo_cardinality_results',
    ...
    """Run ProblemQAOA on cardinality constraints paired with QUBOs."""
    os.makedirs(result_dir, exist_ok=True)
    df = pd.DataFrame()
    df.to_pickle(f'{result_dir}{result_file}.pkl')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run QAOA for cardinality constraints.')
